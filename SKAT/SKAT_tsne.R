...
genotype_matrix <- read.big.matrix(
  ...
  sep = ",",
  header = FALSE,
  backingfile = "genotype_skat_backing.bin", # this writes a backup file to disc, chunks will be read into main memory from this binary file
  descriptorfile = "genotype_skat_desc.desc", # a metadata file will also be written to disc
  type = "integer"
)
...
covariate_matrix <- fread("/data/cephfs-1/work/projects/stoneformers-geneburden/stoneformers/STEP_AFTER_MEETING/TSNE/tSNE.txt")
...
for(i in 1:num_indices){
  ...
  start <- start_indices[i]
    ...
    skat_result <- SKAT(genotype_matrix_chunk,obj)
...
print(output_matrix)
write.table(output_matrix, file = "/data/cephfs-1/work/projects/stoneformers-geneburden/stoneformers/STEP_AFTER_MEETING/SKAT/SKAT_tsne_output_matrix.txt", row.names = FALSE, col.names = FALSE)
print('all done')
